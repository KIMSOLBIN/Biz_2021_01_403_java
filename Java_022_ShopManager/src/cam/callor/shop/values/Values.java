package cam.callor.shop.values;
...
public class Values {
	 ...
	 *  변수에 직접 접근할수 있다.
	 *  
	 *  이 프로젝트가 Run 될때 JVM 에 의해서
	 *  자동으로 생선되고 사용할 준비가 된다.
	 *  
	 *  프로젝트 전체에서 한번만 생성된고
	 *  공유 할 수 있다.
	 *  
	...
	// final를 사용하여 다른곳에서 공유해서 내용이 변경되지 않도록함.
	
	/*
	 * Menu System에서 사용할 각 항목의 item 값을
	 * static ㅂ녀수로 선언을 해 두었다.
	 * 
	 * 각 항목의 item값에 따라 여러 기능을 선택해야하는
	 * 코드가 여기저기 나타난다면 코드를 작성하면서
	 * item값을(숫자) 일일이 기억해야만 한다.
	 * 이러한 상환은 item값을 잘못 지정하여
	 * 논리적 오류가 발생할 소지가 많다.
	 * 
	 * 이때 item 값들을  static 변수로 미리 만들어두고
	 * 필요한 곳에 적용을 하면
	 * 코드를 작성하는 도중에 ㅁ누법적인 검사가 이루져
	 * 논리적 오류를 피할 수 있다.
	 */
