...
import cam.callor.shop.model.CartVO;
import cam.callor.shop.service.CartServiceV2;
...
public class CartServiceImplV2 extends CartServiceImplV1 implements CartServiceV2 {
	...
	public CartServiceImplV2() {
		...
		cartFileName = "src/cam/callor/shop/cartlist.csv";
	}

	/*
	 * CartList에 저장된 상품리스트를 ".../cartlist.txt"파일에 저장
	 * 
	 * 어떤 데이터를 파일에 저장하기 1. 파일이름(경로포함)을 결정하고 2. 파일을 생성 3. 모든 데이터를 문자열로 만들고 4. 파일에
	 * 기록한다. 5. 파일을 연결을 정상적으로 종료한다
	 */
	@Override
	public void saveCartToFile() {
		 ...
		 * 
		 * => 운영체제와 통신을 하여 파일에 저장할 준비를 한다 이때. 운영체제와 통신을 시작하는 아주 짧은 시간동안 생성하고자 하는 파일의 각종
		 * 정보들에 문제가 발생하여 exception이 발생할 수 있다
		 * 
		 * 아무리 코드를 견고하게 작성하여도 이 exception은 완전하게 피할수 없다
		 * 
		 * Java에서는 이처럼 도저히 완전하게 exception을 피할수 없는 경우는 코드를 "반드시 try 예외처리"를 하도록 강제한다 따라서
		 * FileWriter 객체를 생성할때는 반드시 try...catch로 묶어야한다
		 */
		try {
			...
			for (int i = 0; i < nSize; i++) {
				...
				printer.printf("%s,%s,%d,%d,%d\n", cartVO.getUserName(), cartVO.getProductName(), cartVO.getQty(),
						cartVO.getPrice(), cartVO.getTotal());
			}
